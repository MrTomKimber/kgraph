...
def venn_partitions(sets : dict[str,set], 
                    notation_style : str):
    ...
    for k in range(1, n + 1):
        for combo in combinations(range(n), k):
            ...
            if notation_style=='strict':
                ...
                if len(out_sets)!=0:
                    notation=notation + f" ∩ {" ∩ ".join([f"{set_labels[s]}ᶜ" for s in out_sets])}"
...
class RDF2dict():
    ...
    def _infer_ontology_list(self):
        ...
        candidate_ontology_set = set([RDF2dict.get_base_uri(c) for c in relations.union(types)])
        return candidate_ontology_set

    def _update_entities_relations_from_triple(self, 
                                               triple, 
                                               order : int):
        s,p,o = triple
...
#            self.entities[o]._observed_rdf_terms.add(type(RDFS.Class))
        ...
        for identifier, entity in self.entities.items():
            ...
            if not isinstance(identifier, Literal):
                if RDF.type in entity_relations_d:
                    self.entities[identifier]._observed_rdf_classes = entity._observed_rdf_classes.union(entity_relations_d[RDF.type])
            for relation,obj_set in entity_relations_d.items():
                self.relations[relation]._observed_rdf_subject_classes=self.relations[relation]._observed_rdf_subject_classes.union(entity._observed_rdf_classes)
                for obj_pointer in obj_set:
                    obj=self.entities[obj_pointer]
                    self.relations[relation]._observed_rdf_object_classes=self.relations[relation]._observed_rdf_object_classes.union(obj._observed_rdf_classes)
    ...
    def __init__(self, 
        ...
        self.relations={} # A dictionary keyed on predicates - provides graph-level meta-data over the observed usage of the predicate
        self.entities={} # A dictionary keyed on subject - this differs from the raw_graph_dict in that it collects observations across the graph
    ...
    def update_cache(self):
        ...
        cache_update_ontologies = {k:k for k in inferred_required_ontologies-cache_ontologies}
    ...
    def get_entities_pending_metadata(self):
        ...
        undef_classes = all_classes-set([k for k,v in self.entities.items() if v.interactions!={}])
        entities_pending_metadata = undef_relationships.union(undef_classes)
        return entities_pending_metadata

    def _enrich_metadata_for_entities(self,
                        entity_list : set[URIRef], 
                        o_graph : Graph,
                        order):
        triple_count=0
