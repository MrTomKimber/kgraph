...
class GraphEntity(object):
    ...
    def get_outgoing_linked_entity_data(self): # pyright: ignore[reportUndefinedVariable]
        ...
        if self.type=='object':
            for p in predicate_list:
                subject = self.uri
                link_paths.append((p,list(self.graph.objects(subject, p, unique=True))))
        ...
        for candidate_object in link_pointers:
            if candidate_object in self.entity_store.keys():
                link_objects_dict[candidate_object]=self.entity_store[candidate_object]
        ...
        for candidate_object in remaining_pointers:
            if candidate_object not in link_objects_dict.keys():
                new_entity = GraphEntity(self.graph, candidate_object, entity_store=self.entity_store)
    ...
    def get_incoming_linked_entity_data(self): # pyright: ignore[reportUndefinedVariable]
        ...
        for p in predicate_list:
            link_paths.append((p,list(self.graph.subjects(object=object, predicate=p, unique=True))))
    ...
    def _setup_literal(self):
        ...
        if self.literal.datatype is None:
            types=[PYTHON2XSDDATATYPEMAPPING.get(type(value).__name__, URIRef(type(value).__name__))]
        else:
            types=[self.literal.datatype]

        incoming_predicates = sorted(list(self._get_object_predicates(self.literal)))
        incoming_predicate_labels = [str(self._resolve_to_python_primitive(self._get_result_from_preference_list(t, GraphEntity.__default_human_label_prefs))) for t in incoming_predicates]
    ...
    def _setup_uriref(self):
        ...
        subject_type_labels = [str(self._resolve_to_python_primitive(self._get_result_from_preference_list(t, GraphEntity.__default_human_label_prefs))) for t in subject_types]

        outgoing_predicates = sorted(list(self._get_subject_predicates(self.uri)))
        outgoing_predicate_labels = [str(self._resolve_to_python_primitive(self._get_result_from_preference_list(t, GraphEntity.__default_human_label_prefs))) for t in outgoing_predicates]
    ...
    def get_neighbours(self):
        ...
        for p in self.data.outgoing_predicates:
            for o in self._get_objects(self.uri, p):
                outgoing_linked_neighbours.append((GraphEntity(self.graph, p, self.entity_store), 
        ...
        for p in self.data.incoming_predicates:
            for o in self._get_subjects(self.identifier, p):
                incoming_linked_neighbours.append((GraphEntity(self.graph, p, self.entity_store), 
                                          GraphEntity(self.graph, o, self.entity_store)), 
                                         )        
    ...
    def _get_subject_predicates(self, subject : URIRef)->set[URIRef]:
        return set([p for p in self.graph.predicates(subject=subject, object=None)]) # pyright: ignore[reportReturnType]

    def _get_object_predicates(self, object : Identifier)->set[URIRef]:
        return set([p for p in self.graph.predicates(subject=None, object=object)]) # pyright: ignore[reportReturnType]


    def _get_subject_human_label(self, subject : URIRef)->str:
        h_term = self._resolve_to_python_primitive(self._get_result_from_preference_list(subject, GraphEntity.__default_human_label_prefs))
        return str(h_term)
    
    def _get_subject_technical_label(self, subject : URIRef)->str:
        h_term = self._resolve_to_python_primitive(self._get_result_from_preference_list(subject, GraphEntity.__default_technical_label_prefs))
        return str(h_term)
    ...
    def _get_result_from_preference_list(self, subject : URIRef, preferences : list[URIRef])->Node:
...
class RDFExplorer(object):
    ...
    def __init__(self, rdf_g : rdflibGraph, entity_store : dict[Identifier, 'GraphEntity'] | None = None  )->None:
