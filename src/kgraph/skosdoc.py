...
def extract_skos_concept_scheme_uris(skos_graph: rdflibGraph) -> set[URIRef]:
    concept_scheme_uris = set(
        [
            s
            for s, _, _ in list(
                skos_graph.triples((None, RDF.type, SKOS.ConceptScheme))
            )
        ]
    )
...
def extract_skos_concept_uris_for_concept_scheme(
    skos_graph: rdflibGraph, concept_scheme_uri: URIRef
) -> set[URIRef]:
    concept_uris = set(
        [s for s, _, _ in list(skos_graph.triples((None, RDF.type, SKOS.Concept)))]
    )
    concept_uris_in_scheme = set(
        [
            s
            for s, _, _ in list(
                skos_graph.triples((None, SKOS.inScheme, concept_scheme_uri))
            )
        ]
    )
...
def pull_literals_for_subject(
    skos_graph: rdflibGraph, subject: URIRef
